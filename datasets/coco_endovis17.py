...
class EnvidosDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        self.folder_name = f'instrument_dataset_{self.folder_id}'
        self.img_folder = os.path.join(self.root, self.folder_name, 'images')
        self.ann_folder = os.path.join(self.root, self.folder_name, 'xml')
        self.mask_folder = os.path.join(self.root, self.folder_name, 'instruments_masks')

        # img_name = os.listdir(self.img_folder)[file_idx]
        ...
        if self.mode == 'val':
            img_id = torch.tensor([int(1800 + id)])
...
def build(image_set, args):
    ...
    dataset = EnvidosDataset(root, transforms=make_coco_transforms(image_set), mode=image_set)
    return dataset
...
class EndovisCOCO(COCO):
    def __init__(self, mode, dataset=None, root=None):
        if dataset is None and root is not None:
            dataset = EnvidosDataset(root, transforms=make_coco_transforms(mode), mode=mode)
