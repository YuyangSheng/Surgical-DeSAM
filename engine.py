...
def train_one_epoch(model: torch.nn.Module, seg_model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if use_sam:
            ...
            low_res_masks, pred_masks, iou_predictions = seg_model(samples.tensors, target['boxes'], target['size'].tolist())
...
@torch.no_grad()
def evaluate(model, seg_model, criterion, postprocessors, data_loader, base_ds, device, output_dir, epoch):
    model.eval()
    criterion.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        if use_sam:
            ...
            low_res_masks, pred_masks, _ = seg_model(samples.tensors, output_bboxes, target['size'].tolist(), add_noise=False)
            # pred_masks = (pred_masks > 0.5).float()
            outputs['pred_masks'] = low_res_masks
            
            # calculate losses
            iou = intersection_over_union(low_res_masks.cpu(), target['masks'].cpu())
