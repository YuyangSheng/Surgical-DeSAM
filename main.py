...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--dataset_file', default='endovis17')
    parser.add_argument('--coco_path', type=str)
    parser.add_argument('--endovis_path', type=str, default='./endovis17')
    ...
    parser.add_argument('--detr_weights', default='./weights/detr_weights_1024.pth',
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        lr_scheduler.step()
        if args.output_dir:
            checkpoint_paths = [output_dir / f'ckpt_{epoch:04}.pth']
            # # extra checkpoint before LR drop and every 100 epochs
            # if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % 5 == 0:
            #     checkpoint_paths.append(output_dir / f'checkpoint{epoch:04}_lr{args.lr}.pth')

        test_stats, coco_evaluator = evaluate(
            model_without_ddp, seg_model, val_criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, epoch
        )
